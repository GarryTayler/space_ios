...
export default class Paymentdetail extends React.Component {
    ...
    renderCompleteSave() {
        return (
            <Form style={{ paddingHorizontal: 16, paddingBottom: 25 }}>
                ...
                <View style={[elements.flex_rowAlign , {alignItems:'center' , justifyContent:'space-between' }]}>
                    <Text style={[fonts.familyMedium, fonts.size16, fonts.colorLightBlack]}>서비스이용비용</Text>
                    <Text style={[fonts.familyBold, fonts.size16, {color: '#177a77'}]}>{ number_format(this.state.historyData.cost) }원</Text>
                </View>
    ...
    renderCancel() {
        return (
            <Form style={{ paddingHorizontal: 16, paddingVertical: 25 }}>
                <View style={[elements.flex_rowAlign , {alignItems:'center' , justifyContent:'space-between' }]}>
            ...
            <Container>
                ...
                <Form style={{ paddingHorizontal: 16, paddingTop: 30 }}>
                    ...
                    : (this.state.detailData.type == 8 ?
                        "회송불가"
                    : (this.state.detailData.type == 9 ? 
                        "물품 재보관 신청" 
                    : ( this.state.detailData.type == 10 ? 
                        "수동연장신청" 
                    : (this.state.detailData.type == 11 ? "보관종료" : "") )) ))))))) }
                    </Text>
                    <View style={[elements.flex_rowAlign , {alignItems:'center' , justifyContent:'space-between' , marginBottom: 10}]}>
                ...
                { this.state.detailData.type == 1 ?                    // 보관신청
                    this.renderServiceRequestHistory()
                : (this.state.detailData.type == 2 ?                    // 개별물건 찾기
                    this.renderReturnGoodHistory()
                : (this.state.detailData.type == 3 ?                    // 기간내 재보관
                    this.renderRestore()
                : (this.state.detailData.type == 4 ?                    // 자동연장신청
                    this.renderAutoPay()
                : (this.state.detailData.type == 5 ?                    // 환불(신청취소) -> penalty는 삭감 15시 이후 취소
                    this.renderCancel()
                : (this.state.detailData.type == 6 ?                    // 환불(반송요청)
                    this.renderCancelbox()
                : (this.state.detailData.type == 7 ?                    // 결제취소 ->  15시 이전 취소 
                    this.renderCancelPayment()
                : (this.state.detailData.type == 8 ?                    // 환불(회송불가)
                    this.renderCancelReturn() 
                : (this.state.detailData.type == 9 ?                    // 물품 재보관 신청
                    this.renderRestoreGoodHistory() 
                : (this.state.detailData.type == 10 ?                  //  수동연장신청
                    this.renderExtendPay() 
                : (this.state.detailData.type == 11 ?                  // 보관종료
                    this.renderCompleteSave() : null) ) ) ))))))) }
            </Container>
        );
    }
}
