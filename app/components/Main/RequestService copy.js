...
import { Dimensions, View, Image, TouchableHighlight, BackHandler, FlatList } from 'react-native';
...
class RequestService extends React.Component {
    ...
    fetchBoxListData() {
        performNetwork(this, this.state.params.homeComp, getBoxList()).then((response) => {
            ...
            console.log(response.data.box_list);
            this.updatePrice();
        });
    }
                ...
                <FlatGrid
                    ...
                    <TouchableHighlight activeOpacity={ 1 } underlayColor='#0000' style={{ marginHorizontal: 6 }}>
                        <View style={{ borderBottomWidth: 1, borderBottomColor: '#ededed', paddingVertical: 10 }}>
                            ...
                            <Button style={ [elements.size16, { position: 'absolute', alignSelf: 'flex-end', backgroundColor: 'white' }] }
                                onPress={ () => this.onBoxPressed(index) }>
                                <Image
                                    style={ [elements.size16, { position: 'absolute', alignSelf: 'flex-end' }] }
